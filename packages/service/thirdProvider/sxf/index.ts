...
type SxfChunkItem = { text: string };
type SxfChunkResponse = {
  chunks?: SxfChunkItem[];
  count?: number;
};

/**
 * 调用 sxf 智能分块服务完成「文本→chunk」。服务契约:
...
> => {
  ...
  logger.info('Sxf chunking: routing text-to-chunk to external service', {
    url,
  ...
  let data: SxfChunkResponse;
  try {
    const { data: res } = await axiosWithoutSSRF.post<SxfChunkResponse>(
      url,
    ...
    logger.error('Sxf chunking request failed', {
      url,
      textLength: markdown.length,
      chunkSize,
      timeoutMs,
  ...
  if (!Array.isArray(data?.chunks)) {
    logger.error('Sxf chunking response invalid', {
      url,
      textLength: markdown.length,
      chunkSize,
      durationMs: Date.now() - startedAt,
  ...
  if (chunks.length === 0) {
    logger.error('Sxf chunking response empty', {
      url,
      textLength: markdown.length,
      chunkSize,
      durationMs: Date.now() - startedAt,
  ...
  logger.info('Sxf chunking request completed', {
    url,
    textLength: markdown.length,
    chunkSize,
    durationMs: Date.now() - startedAt,
