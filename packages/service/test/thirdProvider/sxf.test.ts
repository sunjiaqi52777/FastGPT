...
import { chunkBySxf } from '@fastgpt/service/thirdProvider/sxf';
...
describe('chunkBySxf', () => {
  ...
  it('rejects with externalChunkNotConfigured when no url is configured', async () => {
    await expect(chunkBySxf({ text: 'content', chunkSize, timeoutMs })).rejects.toBe(
  ...
  it('checks the url before the blank-text short circuit', async () => {
    await expect(chunkBySxf({ text: '   ', chunkSize, timeoutMs })).rejects.toBe(
      DatasetErrEnum.externalChunkNotConfigured
    );
    expect(mockPost).not.toHaveBeenCalled();
  });
  ...
  it('returns empty without calling the service for blank text', async () => {
    await expect(chunkBySxf({ text: '   ', url, chunkSize, timeoutMs })).resolves.toEqual([]);
    expect(mockPost).not.toHaveBeenCalled();
  });
  ...
  it('rejects with the error code when a chunk item is not a non-empty text', async () => {
    ...
    await expect(chunkBySxf({ text: 'content', url, chunkSize, timeoutMs })).rejects.toBe(
