...
import { splitByExternalChunkService } from '@fastgpt/service/core/dataset/utils/externalChunk';
...
describe('splitByExternalChunkService', () => {
  ...
  it('posts markdown and maps returned chunk texts into dataset chunks', async () => {
    ...
    const result = await splitByExternalChunkService({
      text: '## title\ncontent',
      imageIdList,
      url,
      chunkSize,
  ...
  it('rejects with externalChunkNotConfigured when no url is configured', async () => {
    await expect(
      splitByExternalChunkService({ text: 'content', chunkSize, timeoutMs })
    ).rejects.toBe(DatasetErrEnum.externalChunkNotConfigured);
    expect(mockPost).not.toHaveBeenCalled();
  });

  it('checks the url before the blank-text short circuit', async () => {
    await expect(splitByExternalChunkService({ text: '   ', chunkSize, timeoutMs })).rejects.toBe(
  ...
  it('returns empty without calling the service for blank text', async () => {
    await expect(
      splitByExternalChunkService({ text: '   ', url, chunkSize, timeoutMs })
    ).resolves.toEqual([]);
    expect(mockPost).not.toHaveBeenCalled();
  });

  it('adds Bearer authorization header only when a key is provided', async () => {
    await splitByExternalChunkService({
      text: 'content',
      url,
      key: 'secret',
      chunkSize,
  ...
  it('rejects with the error code when a chunk item is not a non-empty text', async () => {
    ...
    await expect(
      splitByExternalChunkService({ text: 'content', url, chunkSize, timeoutMs })
    ).rejects.toBe(DatasetErrEnum.externalChunkInvalidResponse);
  });
  ...
  it('wraps network/timeout failures with the external chunk failed error code', async () => {
    ...
    await expect(
      splitByExternalChunkService({ text: 'content', url, chunkSize, timeoutMs })
    ).rejects.toBe(DatasetErrEnum.externalChunkFailed);
  });
});
